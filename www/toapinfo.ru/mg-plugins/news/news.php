...
class News extends Actioner{
  ...
  public function addNews($array){
    ...
    $sql = 'INSERT INTO `' . PREFIX . 'mpl_news` SET add_date= '.$date.', ';
  ...
  public function deleteNews(){
    ...
    if(DB::query('DELETE FROM `' . PREFIX . 'mpl_news` WHERE `id`= '.DB::quote($_POST['id']))){
      return true;
    }
    return false;
  }
      ...
      if(strlen($name)){
        ...
        if(in_array($ext, $validFormats)){
          if($size < (1024 * 1024)){
            ...
            if (!$this->createImgPreview($actualImageName, $tmp, 0.3) || !$this->createImgPreview($actualImageName, $tmp, 0.7)) {
              $this->messageError = $this->lang['IMG_NOT_CREATE_MINIATURE'];
              return false;
            }
            ...
            if(move_uploaded_file($tmp, $path.DS.$actualImageName)){
  ...
  private function createImgPreview($name, $tmp, $koef){
    $thumbsDir = SITE_DIR.'uploads'.DS.'news'.DS.'thumbs';
    if (!is_dir($thumbsDir)) {
      if (!mkdir($thumbsDir, 755, true)) {
        return false;
      }
    }
    $percent = $koef * 100;
    ...
    return $upload->_reSizeImage($percent.'_'.$name, $tmp, $widthSet, $heightSet, 'PROPORTIONAL', $thumbsDir.DS);
  }
