    ...
    <section class="mc__assoc-plan">
        <?php foreach ($categories as $index => $category): 
              if (stristr($category['url'], 'kinoklub') !== FALSE) {
                  $category['url'] = str_replace('kinoklub', 'proekt-kinoklub', $category['url']);
                } elseif (stristr($category['url'], 'tavrim') !== FALSE) {
                  $category['url'] = str_replace('tavrim', 'tavricheskie-misterii', $category['url']);
                } elseif
                 (stristr($category['url'], 'supervizii') !== FALSE) {
                  $category['url'] = str_replace('supervizii', 'kurs_supervizii', $category['url']);
                } 
                elseif (stristr($category['url'], 'seminars') !== FALSE) {
                  $category['url'] = str_replace('seminars', 'seminary', $category['url']);
                } ?>
            <?php if ($index < $total - 2): // первые три элемента ?>
                    ...
                    <a class="assoc-plan__blockA" href="<?php echo SITE.'/'.$category['parent_url'].$category['url']; ?>">
                        ...
                        <?php if (!empty($category['image_url'])): ?>
                            <img class="foto" src="<?php echo SITE.$category['image_url']; ?>" alt="<?php echo $category['seo_alt']; ?>" title="<?php echo $category['seo_title']; ?>">
                        <?php else: ?>
                            <?php
