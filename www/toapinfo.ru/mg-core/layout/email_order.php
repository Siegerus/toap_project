...
<?php // echo print_r($data,1); ?>
     ...
     <?php /* if(!USER::getUserInfoByEmail($data['email'])->activity) { ?>
      Перейдите по <?php echo $data['confirmLink'] ?> для подтверждения заказа
       <br>
           <?php }*/?>
           ...
           // если пользователь впервые оформил заказ на сайте, и не имел ранее учетной записи
            ...
            <br><b>Ссылка для подтверждения регистрации:</b> <a href="<?php echo $data['link']; ?>" style="color:#1E7EC8;" target="_blank"> Подтвердить</a>
           <?php }?>
      </p>
      <?php
        if (
          !in_array($data['paymentId'], [3, 4]) &&
          (
            $data['propertyOrder']['paymentAfterConfirm'] !== 'true' ||
      ...
      <p class="confirm-info" style="font-size:12px;margin:0 0 10px 0; text-align: center;">
        <a
          ...
          "
          href="<?php echo SITE.'/order?p='.$data['payHash']; ?>"
        >Оплатить заказ сейчас</a>
      </p>
      <?php endif; ?>
    ...
    <tr>
      <td>
        ...
        <?php 
        // Вывод стоимости доставки в письме пользователю
