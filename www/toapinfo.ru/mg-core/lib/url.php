...
class URL {
  ...
  private function __construct() {
    ...
    $route = ($route || $route === '0') ? $route : 'index';
    ...
    // Здесь довольно непростая комбинация условий, но она верная и лучше её не трогать
    // Это условие можно расшифровать как "Если запрос в админку, на ajax или на previewer, то не защищаем от XSS, в остальных случаях защищаем",
    // а дополнительная проверка на ajaxrequest прописана, т. к. ajax это подстрока ajaxrequest, а проверка проходит через strpos
    ...
    $this->queryParams = !empty($_REQUEST)?$_REQUEST:array();
  }
  ...
  public static function isSection($section) {
    ...
    //return  (isset($sections[1]) && ($sections[1] == $section)) ? true : false;
  }
    ...
    if (($p = strpos($res, '?')) !== false) {
      ...
      foreach ($params as $value) {
        ...
        if (isset($paramsarr[$tmp[0]])) {
          ...
          $paramsarr[$tmp[0]] = isset($tmp[1])?strval($tmp[1]):'';
        }
      }
  ...
  public static function prepareUrl($str, $product = false, $toLower = true) {
    ...
    $str = urlencode($str);
    $str = str_replace(["%2F","%3A","%3F","%3D","%26"], ["/",":","?","=","&"], $str);
  ...
  public static function getUrlRedirect(){
    ...
    if(URL::getClearUri() == '/modificator' || MG::getSetting('useSeoRedirects') != 'true'){
      return $redirect;
    }
  ...
  public static function isAdminAjax() {
    ...
    $lastSection = self::getLastSection();
    if ($lastSection === 'ajax' || $lastSection === 'ajaxrequest') {
      // Проверка, что запрос пришел из административной панели
      if (!empty($_SERVER['HTTP_REFERER'])) {
        $refererArray = parse_url($_SERVER['HTTP_REFERER']);
        $refererPath = rtrim($refererArray['path'], '/');
        if (mb_substr($refererPath, -strlen('mg-admin')) === 'mg-admin') {
          return true;
        }
      }
    }
