...
class User {
  ...
  public static function access($zone = '') {
    ...
    $thisRole = empty(self::getThis()->role)?'2':self::getThis()->role;
    if($zone == 'admin_zone'){
      $sql = 'SELECT * FROM `'.PREFIX.'user` WHERE `role` = '.DB::quote($thisRole).' AND `login_email` = '.DB::quote($_SESSION['user']->login_email).' AND `id` = '.DB::quote($_SESSION['user']->id);
      $res = DB::query($sql);
  ...
  private static function saveLogin($userId) {
