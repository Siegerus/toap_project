...
class Urlrewrite{
  ...
  private function __construct() {
    ...
    if($arInfo = self::getUrlRewriteData()) {
      ...
      if (isset($arInfo['short_url'])) {
        MG::set('urlrewrite', $arInfo['short_url']);
        if (!empty($_GET['search'])) {
          // Если в реврайте есть поиск, то проставляем его в объекте класса URL, чтобы он работал на "страницах с применёнными фильтрами"
          URL::setQueryParametr('search', $_GET['search']);
        }
      }
  ...
  public static function getUrlRewriteData($id = 0) {
    ...
    $where = '';
    $uri = URL::getClearUri();
    if(strpos($uri, 'mg-admin/ajax') === false && strpos($uri, 'mgadmin/ajax') === false && ($uri == '/modificator' || MG::getSetting('useSeoRewrites') != 'true')) {
      $args = func_get_args();
      return MG::createHook(__CLASS__."_".__FUNCTION__, $urlRewriteData, $args);
    }

    if($id > 0) {
      $where = '`id` = '.DB::quoteInt(intval($id), true);
    } else {
      $url = URL::getClearUri();
      $url = trim($url, "/");  
  ...
  public static function setUrlRewrite($arFields) {
    ...
    if(!empty($arFields['id'])) {
      $sql = "UPDATE `".PREFIX."url_rewrite` SET ".DB::buildPartQuery($arFields)
              ." WHERE `id` = ".DB::quoteInt($arFields['id'], true);
  ...
  public static function setActivity($id, $activity) {
    $sql = "UPDATE `".PREFIX."url_rewrite` "
            ."SET `activity` = ".DB::quoteInt($activity, true)." "
            ."WHERE `id` = ".DB::quoteInt(intval($id), true);
