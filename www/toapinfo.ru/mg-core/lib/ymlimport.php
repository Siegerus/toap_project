...
class ymlImport{
  ...
  public static function parseProductsToDb($file, $priceModifier, $onlyPrice, $importCategory, $importInCategory, $newFileStructure, $onlyNewProductsImages = false) {
    $startTime = microtime(true);
    ...
    while ($xmlReader->name === $productXmlNodeName) {
      ...
      $currencyId = ($productXml->currencyId == "RUB") ? "RUR" : $productXml->currencyId;
      ...
      if($onlyPrice && $findProduct) {
        ...
        if($findProduct) {
          ...
          if (isset($arAddProductFields['count'])) {
            $arAddProductFields['count'] = round($arAddProductFields['count'],2);
          }
    ...
    while (true) {
      ...
      foreach($rawImages as $picture) {
        ...
        if (strpos($fileType, 'image/') !== false) {
          $upload->_reSizeImage('70_'.$imageName, 'uploads/'.$newFile, $widthPreview, $heightPreview);
          $upload->_reSizeImage('30_'.$imageName, 'uploads/'.$newFile, $widthSmallPreview, $heightSmallPreview);
