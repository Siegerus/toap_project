...
class Models_Order {
    ...
    public function isValidData($arrayData, $require = array('email','phone','payment'),$createUser = true , $error = null) {
        ...
        if(in_array('email', $require)&&(MG::getSetting('requiredFields')=='true')) {
          // Корректность емайл.
          if (!MG::checkEmail($arrayData['email'])) {
...
//mg::loger($adminOrder);
        ...
        if (!empty($_POST['yur_info']) && $_POST['customer'] === 'yur') {
        ...
        if ($id) {
            ...
            $adminSubject = 'Оформлена заявка №'.$orderNumber.'. ';
  ...
  public function deleteOrder($id, $arrayId = null) {
        ...
        if ($returnArray) {
          ...
          WHERE `delivery_id` '.$in);
            ...
            foreach ($deliveryArray as &$item) {
                if (MG::getSetting('useNameParts') == 'true') {
                  $item['name_parts'] = '1';
                } else {
                  $item['name_parts'] = '0';
                }
    ...
    public function printOrder($id, $sign = true, $type="order", $usestamp='false') {
        ...
        if (isset($_SESSION['tempRequisites'])) {
            $addReqs = MG::getSetting('addRequisites',true);
            if (isset($addReqs[$_SESSION['tempRequisites']])) {
                foreach ($addReqs[$_SESSION['tempRequisites']] as $key => $value) {
