	...
	return {
		...
		createdSizeVariant: function() {
			...
			for(row = 0; row < variantRowA.length; row++) {
				...
				if(toDel) {
					if(variantRowA.length > 0 && $('.variant-row').length > 1)
				...
				for(i = -1; i <= color.data.length; i++) {
					...
					for(j = -1; j <= size.data.length; j++) {
						...
						if((i >= 0)&&(j < 0)&&(i != color.data.length)) {
							if(color.data[i].img != '') {
								table += '<td class="color" style="padding:2px;">\
							...
							if((i >= 0)&&(j == size.data.length)&&(i != color.data.length)) {
								...
								table += '<td style="padding:5px 9px;">'+checkbox+'</td>';
							}
							...
							if(((j < size.data.length)&&(i < color.data.length))||((j == -1)&&(i == color.data.length))||((i == -1)&&(j == size.data.length))) {
		...
		createUserFields: function(container, userFields, allProperty, propertyGroup) {
			...
			function buildElements(property, index, array) {
				...
				if (property.type == 'file') {
					...
					var link = mgBaseDir + value;
					var fileName = value.split('/').pop();

					let fileSelectStyle = '';
					let fileStyle = 'style="display: none;"';
					if (value) {
		...
		addFileToProp: function(file, id) {
			...
			const propInput = $('input.property[data-id="'+id+'"');
			if (!propInput.length) {
				admin.indication('error', 'Неизвестная ошибка');
				return false;
			}
			propInput.val(filePath);
			$('.property-file-select[data-id="'+id+'"]').hide();
			$('.property-file-name-container[data-id="'+id+'"]').show();
			$('.property-file-name[data-id="'+id+'"]').attr("href", fileUrl);
			$('.property-file-name[data-id="'+id+'"] span').html(fileName);
		},

		removeFileFromProp: function(id) {
			const propInput = $('input.property[data-id="'+id+'"');
			if (!propInput.length) {
				admin.indication('error', 'Неизвестная ошибка');
				return false;
			...
			propInput.val('');
			$('.property-file-select[data-id="'+id+'"]').show();
			$('.property-file-name-container[data-id="'+id+'"]').hide();
		}
