...
';
  ...
  public function ordersUpdate($filename)
  {
    ...
    while($xmlReader->read()) {
      if (
        ...
        if (empty($orderId) || empty($orderStatusId) || $orderStatusId == $currentStatusId) {
          continue;
        }
  ...
  public function processImportXml($filename)
  {
    ...
    if (file_exists('uploads/variants1C.txt') && is_writable('uploads') && (stristr($filename, 'import') !== FALSE)) {
          ...
          if (empty($ids1c[1])) {
            ...
            if ($row = DB::fetchArray($dbRes)) {
              ...
              if (!empty($offerXml->Картинка[0])) {
                ...
                $imagesTempcmlPath = TEMP_DIR.'tempcml';
  ...
  public function deleteRootCat() {
    ...
    if ($rootCatDataRow = DB::fetchAssoc($rootCatDataResult)) {
      ...
      $fixParentUrlSql = 'UPDATE `mg_category` '.
