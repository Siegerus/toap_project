...
class Controllers_Cart extends BaseController {
      ...
      if (!empty($_REQUEST['count_change'])) {
        ...
        foreach ($deliverys as $value) {
          ...
          if ($value['free'] > 0 && $totalOrderPrice > $value['free']) {
            $deliv[] = 0;
            $delivExtand[$value['id']] = 0;
          }
          else{
            $deliv[] = MG::numberFormat((float)$value['cost']);
            $delivExtand[$value['id']] = MG::numberFormat((float)$value['cost']);
          }
        }
  ...
  public function updateCart() {
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        if ($item['propertySetId'] == $_POST['propertySetId'] && $item['id'] == $_POST['inCartProductId']) {
          $_SESSION['cart'][$key]['count'] = (is_numeric($_REQUEST['amount_input'])) ? 
                  floatval($_REQUEST['amount_input']) : 1;
        }
      }
      $response = array(
        'status' => 'success',
   ...
   if (
    empty($_POST)
    && isset($_REQUEST['inCartProductId']) 
    || 
    (isset($_POST['updateCart']) && isset($_POST['inCartProductId']) && (($_POST['ajax'] === 'buyclickflag' && count($_POST) == 4) ||count($_POST) == 3 || count($_POST) == 2))
     ) {
    ...
    $result = $cart->addToCart($_REQUEST['inCartProductId'], isset($_REQUEST['amount_input'])?floatval($_REQUEST['amount_input']):1, $property, $variantId);
    if ($result) {
      $response = array(
        'status' => 'success',
        'data' => SmalCart::getCartData()
  ...
  public function delFromCart() {
    $cart = new Models_Cart;
    $property = null;
    if (isset($_POST['property']) && isset($_SESSION['propertySetArray'][$_POST['property']])) {
      $property = $_SESSION['propertySetArray'][$_POST['property']];
    }
    
    $cart->delFromCart($_POST['itemId'], $property, isset($_POST['variantId'])?$_POST['variantId']:null);
