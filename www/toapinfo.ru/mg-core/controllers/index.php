...
class Controllers_Index extends BaseController {
  ...
  function __construct() {
    ...
    $whereCondition = (MG::getSetting('printProdNullRem') == 'true') ? ' AND (pv.count != 0 || p.count != 0)' : '';
    ...
    if (empty($dataGroupProducts)) {
      ...
      foreach ($productsList['catalogItems'] as &$item) {
        ...
        if (NULL_OLD_PRICE && (MG::numberDeFormat($item["price_course"]) > MG::numberDeFormat($item['old_price']))){
          $item['old_price'] = 0;
        }
        ...
        $blocksVariants = $product->getBlockVariants($item['id'], 0, defined('TEMPLATE_INHERIT_FROM'));
